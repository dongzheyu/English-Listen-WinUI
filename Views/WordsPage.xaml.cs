    ...
    public sealed partial class WordsPage : Page
    {
            ...
            try
            {
                ...
                var wordItems = await Task.Run(() =>
                {
                    return wordsText.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
                        ...
                        .Select(w => new WordItem
                            { Word = w, Translation = _translationLibrary.GetTranslation(w) ?? "" })
                        .ToList();
                });
        ...
        private async void LoadGroupButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            var scrollViewer = new ScrollViewer
                { VerticalScrollBarVisibility = ScrollBarVisibility.Auto, MaxHeight = 400 };
            ...
            var selectButtonsPanel = new StackPanel
            {
                ...
                Margin = new Thickness(0, 5, 0, 0), Spacing = 10
            };

            var selectAllButton = new Button { Content = "全选", Width = 80, Height = 30 };
            selectAllButton.Click += (s, args) => { wordlistListView.SelectAll(); };
        ...
        private async void CreateGroupButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (result == ContentDialogResult.Primary)
            {
                ...
                // 创建新分组
                var newGroup = new WordListGroup
                {
        ...
        private async void EditGroupButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            var selectAllButton = new Button { Content = "全选", Width = 80, Height = 30 };
            selectAllButton.Click += (s, args) => { fileListView.SelectAll(); };
            selectButtonsPanel.Children.Add(selectAllButton);
        ...
        private async void WordlistManagementButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Orientation = Orientation.Horizontal, HorizontalAlignment = HorizontalAlignment.Center,
                Margin = new Thickness(0, 5, 0, 5), Spacing = 10
            };
            ...
            // 加载词库按钮
            var loadWordlistButton = new Button
                { Content = "加载选中的词库", Width = 150, Height = 35, Margin = new Thickness(0, 5, 0, 0) };
            ...
            var buttonsPanel1 = new StackPanel
            {
                Orientation = Orientation.Horizontal, HorizontalAlignment = HorizontalAlignment.Center, Spacing = 10
            };

            var loadGroupButton = new Button { Content = "加载分组", Width = 100, Height = 35, Margin = new Thickness(5) };
            loadGroupButton.Click += async (s, args) =>
            {
                dialog.Hide();
                await Task.Delay(100);
                LoadGroupButton_Click(s, args);
            };
            buttonsPanel1.Children.Add(loadGroupButton);

            var createGroupButton = new Button
                { Content = "创建分组", Width = 100, Height = 35, Margin = new Thickness(5) };
            createGroupButton.Click += async (s, args) =>
            {
                dialog.Hide();
                await Task.Delay(100);
                CreateGroupButton_Click(s, args);
            };
            buttonsPanel1.Children.Add(createGroupButton);

            stackPanel.Children.Add(buttonsPanel1);

            var buttonsPanel2 = new StackPanel
            {
                Orientation = Orientation.Horizontal, HorizontalAlignment = HorizontalAlignment.Center, Spacing = 10
            };

            var editGroupButton = new Button { Content = "编辑分组", Width = 100, Height = 35, Margin = new Thickness(5) };
            editGroupButton.Click += async (s, args) =>
            {
                dialog.Hide();
                await Task.Delay(100);
                EditGroupButton_Click(s, args);
            };
            buttonsPanel2.Children.Add(editGroupButton);

            var deleteGroupButton = new Button
                { Content = "删除分组", Width = 100, Height = 35, Margin = new Thickness(5) };
            deleteGroupButton.Click += async (s, args) =>
            {
                dialog.Hide();
                await Task.Delay(100);
                DeleteGroupButton_Click(s, args);
            };
            ...
            var deleteWordlistButton = new Button
            {
                ...
                Background = new SolidColorBrush(Color.FromArgb(255, 255, 68, 68)),
                Foreground = new SolidColorBrush(Color.FromArgb(255, 255, 255, 255))
            };
            deleteWordlistButton.Click += async (s, args) =>
            {
                var selectedFiles = wordlistListView.SelectedItems.Cast<string>().ToList();
                if (selectedFiles.Count == 0)
                {
                    MainWindow.ShowNotification("请选择要删除的词库文件");
                    return;
                }
                ...
                {
                    Title = "确认删除",
                    Content = $"确定要删除选中的 {selectedFiles.Count} 个词库文件吗？此操作不可恢复。",
                    PrimaryButtonText = "删除",
                    CloseButtonText = "取消",
                    XamlRoot = this.XamlRoot
                };
                ...
                try
                {
                    ...
                    {
                        var filePath = Path.Combine(wordlistDir, fileName);
                        if (File.Exists(filePath))
                        {
                    ...
                    await _viewModel.LoadWordListFilesAsync();
                    PopulateWordList();
                    MainWindow.ShowNotification($"成功删除 {deletedCount} 个词库文件");
                }
                catch (Exception ex)
                {
            ...
            // 导入导出
            var buttonsPanel3 = new StackPanel
            {
                Orientation = Orientation.Horizontal, HorizontalAlignment = HorizontalAlignment.Center, Spacing = 10,
                Margin = new Thickness(0, 10, 0, 0)
            };
        ...
        private async void WordOperationsButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            deleteSelectedButton.Click += async (s, args) =>
            {
                dialog.Hide();
                await Task.Delay(100);
                DeleteSelectedButton_Click(s, args);
            };
            buttonsPanel.Children.Add(deleteSelectedButton);

            var clearButton = new Button { Content = "清空列表", Width = 110, Height = 35 };
            clearButton.Click += async (s, args) =>
            {
                dialog.Hide();
                await Task.Delay(100);
                ClearButton_Click(s, args);
            };
        ...
        private void DeleteSelectedButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Debug.WriteLine($"删除选中项错误: {ex.Message}");
            }
        }
        ...
        private void ApplyFilter()
        {
            ...
            {
                var originalLines = _originalWordsText
                    .Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
                    .Select(w => w.Trim())
                    .Where(w => !string.IsNullOrEmpty(w))
                    .Select(w => new WordItem { Word = w, Translation = "" })
            ...
            {
                filteredLines =
                    filteredLines.Where(line => line.Contains(searchText, StringComparison.OrdinalIgnoreCase));
            }
            ...
            _wordList = filteredLines.Select(w => w.Trim()).Where(w => !string.IsNullOrEmpty(w)).Select(w =>
                new WordItem { Word = w, Translation = _translationLibrary.GetTranslation(w) ?? "" }).ToList();
            UpdateWordsListBox();
        }
            ...
            try
            {
                ...
                var result = await Task.Run(async () =>
                {
                    ...
                    foreach (var word in selectedItems)
                    {
                        ...
                        if (translated % 10 == 0 || translated + skipped == totalCount)
                        {
                            ...
                            _ = DispatcherQueue.TryEnqueue(() => { LoadingStatusText.Text = progress; });
                        }
                    }
                ...
                if (result.limitHit)
                {
                    ...
                    MainWindow.ShowNotification(
                        $"翻译限额已用完（今日剩余 {remaining} 个），{result.skipped} 个单词被跳过。成功翻译 {result.translated} 个");
                }
                else
                {
            ...
            try
            {
                ...
                if (files.Count > 0)
                {
                    ...
                    foreach (var file in files)
                    {
                        ...
                        {
                            Debug.WriteLine($"复制文件失败: {file.Name} - {ex.Message}");
                        }
                    }
        ...
        private async void DeleteWordlistButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (result == ContentDialogResult.Primary)
            {
                try
                {
                    ...
                    var settingsService = _viewModel.GetType()
                        .GetField("_settingsService", BindingFlags.NonPublic | BindingFlags.Instance)
                        ?.GetValue(_viewModel);
        ...
        private async void ExportButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (result == ContentDialogResult.Primary)
            {
                ...
                {
                    // 获取词库目录路径
                    var wordlistDir = _viewModel.GetType().GetProperty("WordlistDirectory")?.GetValue(_viewModel)
                        ?.ToString();
                    if (string.IsNullOrEmpty(wordlistDir))
                    {
        ...
        private async void AppendToWordlistButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (result == ContentDialogResult.Primary)
            {
                ...
                try
                {
                    ...
                    var wordlistPath = Path.Combine(_viewModel.Settings.GetWordlistDirectory(), selectedWordlist);

                    // 读取现有词库内容
                    var existingWords = new List<string>();
                    if (File.Exists(wordlistPath))
                    {
                        existingWords = File.ReadAllLines(wordlistPath)
                            .Where(line => !string.IsNullOrWhiteSpace(line))
                            .Select(line => line.Trim())
                            .ToList();
                    }
                    ...
                    // 保存词库文件
                    File.WriteAllLines(wordlistPath, existingWords);
        ...
        {
            public DictationTestParamsWithTranslations(List<DictationTestPage.WordTranslationPair> wordList,
                bool randomOrder = false, bool readTranslation = false, bool isPaperMode = false)
            {
                WordList = wordList;
            ...
            public List<DictationTestPage.WordTranslationPair> WordList { get; set; } =
                new List<DictationTestPage.WordTranslationPair>();
